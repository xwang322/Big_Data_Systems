...
if __name__ == "__main__":
    conf = SparkConf()
    conf.setAppName("CS-838-Assignment2-PartA-3")
    ...
    links = lines.map(lambda urls: Neighbors(urls)).distinct().groupByKey().persist()
    ranks = links.map(lambda url_neighbors: (url_neighbors[0], 1.0)).persist()
