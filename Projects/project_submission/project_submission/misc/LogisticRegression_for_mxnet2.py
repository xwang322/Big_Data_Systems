...
def TFRecordIter():
    ...
    with g.as_default():
        ...
        return dataset_train, dataset_train_labels, dataset_test, dataset_test_labels			
...
for i in range(num_iteration):
    print "this is the {0} iteration".format(i+1)
    returnlist = TFRecordIter()
    train = mx.io.NDArrayIter(data=returnlist[0], label=returnlist[1], batch_size=batch_size, shuffle=False)
    test = mx.io.NDArrayIter(data=returnlist[2], label=returnlist[3], batch_size=batch_size, shuffle=False)
